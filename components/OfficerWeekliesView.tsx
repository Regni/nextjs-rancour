...
const OfficerWeekliesView = async () => {
  ...
  const raiderWithoutWeekies = weeklyRoster.filter((weeklieEntry: any) => weeklieEntry.completed ===false)
  const raiderWithWeekies = weeklyRoster.filter((weeklieEntry: any) => weeklieEntry.completed ===true)

  return (
    <section>
      <h3>Raider that didn't do their weeklies</h3>
      <Separator className="my-2"/>
      <RosterAnimationComponent roster={raiderWithoutWeekies} officerPage = {true}/>
      <Separator className="my-2" />
